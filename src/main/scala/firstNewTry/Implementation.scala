...
case class Implementation(methodName: String,
  ...
  def timeOnOwn: Option[BigOLiteral] = implementation.timeOnOwn

  def timeWith(otherCosts: Map[String, BigOLiteral]): Option[BigOLiteral] = {
...
abstract class ImplementationExpr {
  ...
  def timeWithSubstitutions(substitute: MethodUse => Option[BigOLiteral]): Option[BigOLiteral] = this match {
    case mu: MethodUse => substitute(mu)
    case Sum(args) =>
      val subs = args.map(_.timeWithSubstitutions(substitute))
      subs.reduce((xOpt: Option[BigOLiteral], yOpt: Option[BigOLiteral]) => (xOpt, yOpt) match {
  ...
  lazy val timeOnOwn: Option[BigOLiteral] = {
...
case class Constant(bigO: BigOLiteral) extends ImplementationExpr
