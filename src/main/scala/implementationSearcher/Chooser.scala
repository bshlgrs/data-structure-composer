...
/**
  ...
  */
object Chooser {
  type SearchResultMap = Map[Impl, Impl]
  // x[f] if x.foo <- 1
  // y[g] <- x[g]
  // should infer
  // y[g] if g.foo <- 1
...
//  )
  ...
  def getAllTimesFromEmpty(impls: Set[Impl], library: ImplLibrary, freeVariables: Set[MethodName]): UnfreeImplSet = {
    getAllTimes(UnfreeImplSet(Map(), freeVariables, library.decls), impls, library)._1
  }

  def getAllTimes(initialUnfreeImplSet: UnfreeImplSet,
                  impls: Set[Impl],
                  implLibrary: ImplLibrary): (UnfreeImplSet, SearchResultMap) = {
    val queue = mutable.Set[Impl]()
    val usedImpls = mutable.Map[Impl, Impl]()
  ...
  def getRelevantTimesForDataStructures(implLibrary: ImplLibrary,
                                        structures: Set[DataStructure]): (UnfreeImplSet, SearchResultMap) = {
    ...
    val (bestReadImplementations: UnfreeImplSet, readSources: SearchResultMap) = getAllTimes(
    ...
    val allWriteImplementations: Set[UnfreeImplSet] = structures.map((s) =>
      getAllTimes(bestReadImplementations, s.writeMethods ++ implLibrary.writeMethods, implLibrary)._1)
    ...
    (bestReadImplementations.addImpls(combinedWriteImplementations.allImpls), readSources)
  }
  ...
  def allParetoOptimalDataStructureCombosForAdt(library: ImplLibrary,
    ...
    }).toSet : Set[(Set[(String, DataStructure)], (UnfreeImplSet, SearchResultMap))]

    val choicesSet: Set[DataStructureChoice] = results.flatMap({
      case (set: Set[(String, DataStructure)], (uis: UnfreeImplSet, srm: SearchResultMap)) => {
        val methods = adt.methods.keys.map((methodExpr: MethodExpr) => {
          // TODO: let this be a proper dominance frontier
          methodExpr ->
            uis.implsWhichMatchMethodExpr(methodExpr, ParameterList.empty).headOption.map(_.withName(methodExpr.name))
        }).toMap

        if (methods.forall(_._2.isDefined))
          Set[DataStructureChoice](
            DataStructureChoice.buildWithSources(
              set,
